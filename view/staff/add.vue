<template>
	<view class="main-container tn-safe-area-inset-bottom tn-bg-gray--light">
		<!-- 顶部自定义导航 -->
		<uni-nav-bar statusBar class="custom-nav" :fixed="true" :border="false" title="添加店员" left-icon="left"
		...
		<view class="tn-margin-top-xs">
			<scroll-view class="tn-store tn-bg-white tn-padding-left tn-padding-right" scroll-y
				...
				<tn-form :model="staffInfo" ref="staffInfoRef" :errorType="['message']" :labelWidth="180"
					...
					<tn-form-item label="登录密码：" prop="password" required>
						<tn-input v-model="staffInfo.password" type="password" placeholder="请输入登录密码" />
					</tn-form-item>
		...
		methods: {
			...
			getStoreList() {
				store.storeList().then(res => {
					...
					const store = this.vuexMerchantStoreInfo
					if (store) {
						this.staffInfo.storeNo = store.storeNo
						this.staffInfo.storeName = store.storeName
					}
				})
			},
			getRoleList() {
				role.roleList().then(res => {
					...
					if (this.roleList.length > 0) {
						const roleItem = this.roleList[0]
						this.staffInfo.roleId = roleItem.value
						this.staffInfo.roleName = roleItem.label
					}
				})
			},
			...
			staffSubmit() {
				this.$refs.staffInfoRef.validate(valid => {
					if (valid) {
						staff.accountCreate(this.staffInfo).then(res => {
							this.$refs.tips.show({
								msg: '添加成功',
