<template>
	<view class="main-container tn-safe-area-inset-bottom tn-bg-gray--light">
		<!-- 顶部自定义导航 -->
		<uni-nav-bar class="custom-nav" statusBar :fixed="true" :border="false" title="修改门店" left-icon="left"
			...
			<scroll-view class="tn-store tn-bg-white tn-padding-left tn-padding-right" scroll-y
				...
				<tn-form :model="storeInfo" ref="storeInfoRef" :errorType="['message']" :labelWidth="180"
					...
					<tn-form-item label="门店地址：" prop="regions">
						...
						<tn-picker mode="region" v-model="regionActionshow" :areaCode='regionAreaCode'
							@confirm="regionActionConfirm"></tn-picker>
					</tn-form-item>
		...
		methods: {
			...
			getStoreInfo() {
				store.storeInfo(this.id).then(res => {
					...
					if (this.storeInfo.province && this.storeInfo.city && this.storeInfo.area) {
						this.regionAreaCode = [this.storeInfo.province, this.storeInfo.city, this.storeInfo.area]
						this.storeInfo.regionName =
							`${this.storeInfo.provinceName} - ${this.storeInfo.cityName} - ${this.storeInfo.areaName} `
					}
				})
			},
			getAccountList() {
			...
			storeSubmit() {
				this.$refs.storeInfoRef.validate(valid => {
					if (valid) {
						store.storeEdit(this.id, this.storeInfo).then(res => {
							this.$refs.tips.show({
								msg: '修改成功',
