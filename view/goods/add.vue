<template>
	<view class="template-product main-container tn-safe-area-inset-bottom tn-bg-gray--light">
		<!-- 顶部自定义导航 -->
		<uni-nav-bar statusBar class="custom-nav" :fixed="true" :border="false" title="新增商品" left-icon="left"
					...
					<view class="tn-padding">
						...
						<tn-form :model="goodsInfo" ref="goodsInfoRef" :labelWidth="100" :errorType="['message']">
							...
							<tn-form-item label="条码: " prop="spuCode">
								<tn-input type="text" v-model="goodsInfo.spuCode" placeholder="不填则自动生成" />
							</tn-form-item>
							...
							<tn-list-view class="tn-margin tn-radius-xs tn-border-solid">
								<tn-list-cell class="specs-cell" v-for="(item,index) in specList" :key="index">
									<view class="tn-flex tn-flex-col-center tn-flex-row-between">
										...
										<view
											class="tn-btn-content__item tn-round tn-text-sm tn-text-bold tn-border-solid tn-border-red tn-color-red"
				...
				<scroll-view scroll-y="true" :style="{height:specPopupHeight+'px'}">
					<view style="padding: 45px 16px 16px">
						<tn-form :model="editSpecInfo" ref="editSpecInfoRef" :errorType="['message']" :labelWidth="180"
							...
							<tn-form-item label="初始库存：" prop="referenceAmount">
								<tn-input v-model="editSpecInfo.sellStock" type="number" placeholder="初始库存" />
							</tn-form-item>
			...
			submitGoods() {
				this.$refs.goodsInfoRef.validate(valid => {
					if (valid) {
						...
						this.goodsInfo.syncStoreNo = this.vuexMerchantStoreInfo.storeNo
						spu.storageCreate(this.goodsInfo).then(res => {
							this.$refs.tips.show({
								msg: '添加成功',
