<template>
	<view class="template-classify tn-safe-area-inset-bottom main-container tn-bg-gray--light">
		<!-- 顶部自定义导航 -->
		<uni-nav-bar :border="false" statusBar :fixed="true" class="custom-nav" title="分类管理">
			<block slot="left">
			...
			<!-- <block slot="right">
				<view class="custom-nav__item">
					<uni-icons class="input-uni-icon" type="plusempty" size="18" color="#999" @click="editCategory()" />
				</view>
			</block> -->
		</uni-nav-bar>
		<view class="tn-classify__wrap">
			<view class="tn-classify__container tn-flex tn-flex-nowrap tn-flex-row-around">
				<scroll-view class="tn-classify__box tn-width-full" scroll-y :scroll-top="scrollViewTop"
					...
					<view class="tn-classify__content tn-padding-top-xs" v-if="categoryList.length>0">
						<tn-list-view unlined="all">
							<block v-for="(item, index) in categoryList" :key="index">
								<tn-list-cell :arrow="true" @click="editCategory(item)">{{item.name}}</tn-list-cell>
							</block>
						</tn-list-view>
					</view>
					<view v-else class="tn-padding-top-xl">
						<tn-empty mode="list" text="暂无分类信息"></tn-empty>
					</view>
				</scroll-view>
			</view>
		</view>
		<view class="edit tnxuanfu" @tap="editCategory()">
			<view
...
<script>
	...
	export default {
		...
		methods: {
			...
			editCategory(item) {
				uni.navigateTo({
					url: `/view/goods/category/edit?id=${item?.id || ''}`
				})
			}
		}
	}
